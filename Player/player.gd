...
var hitpoints: int = max_hitpoints:
	set(value):
		if value < hitpoints:
			damage_animation_player.stop(false) # Reset the animation each time you take damage
...
func _process(delta: float) -> void:
	if Input.is_action_pressed("aim"):
		...
		weapon_camera.fov = lerp(weapon_camera_fov, weapon_camera_fov * aim_multiplier, 20.0)
	else:
...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseMotion:
		if Input.mouse_mode == Input.MOUSE_MODE_CAPTURED:
			mouse_motion = -event.relative * 0.002 # This sets up mouse senativeity
			if Input.is_action_pressed("aim"):
...
func handle_camera_rotation() -> void:
	...
	camera_pivot.rotate_x(mouse_motion.y) # sets up/down movement
